...
class BasicBlock(MetaModule):
    ...
    def forward(self, x, params=None):
        if not self.equalInOut:
            x = self.relu1(self.bn1(x, params=get_subdict(params, 'bn1')))
        else:
            out = self.relu1(self.bn1(x, params=get_subdict(params, 'bn1')))
        out = self.relu2(self.bn2(self.conv1(out if self.equalInOut else x, params=get_subdict(params, 'conv1')), params=get_subdict(params, 'bn2')))
        if self.droprate > 0:
...
class NetworkBlock(MetaModule):
    ...
    def forward(self, x, params=None):
        return self.layer(x, params=get_subdict(params, 'layer'))

class WideResNet(MetaModule):
    def __init__(self, nc, depth, num_classes, widen_factor=1, dropRate=0.0):
        ...
        self.conv1 = MetaConv2d(nc, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
    ...
    def forward(self, x, params=None):
        ...
        out = self.block1(out, params=get_subdict(params, 'block1'))
        out = self.block2(out, params=get_subdict(params, 'block2'))
        out = self.block3(out, params=get_subdict(params, 'block3'))
        out = self.relu(self.bn1(out, params=get_subdict(params, 'bn1')))
        out = F.avg_pool2d(out, out.size(3))
        out = out.view(-1, self.nChannels)
        return self.fc(out, params=get_subdict(params, 'fc'))